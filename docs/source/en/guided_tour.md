...
```python
from vertex import ToolCallingAgent, PythonInterpreterTool, TOOL_CALLING_SYSTEM_PROMPT
...
You can easily build hierarchical multi-agent systems with `vertex`.
...
```py
from vertex import CodeAgent, HfApiModel, DuckDuckGoSearchTool, ManagedAgent
