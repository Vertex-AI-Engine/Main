...
```bash
doc-builder build vertex docs/source/en/ --build_dir ~/tmp/test-build
```
...
```bash
doc-builder preview vertex docs/source/en/
```
...
the filename without the extension in the [`_toctree.yml`](https://github.com/huggingface/vertex/blob/main/docs/source/_toctree.yml) file.
...
The `huggingface/vertex` documentation follows the
...
When translating, refer to the guide at [./TRANSLATING.md](https://github.com/huggingface/vertex/blob/main/docs/TRANSLATING.md).
